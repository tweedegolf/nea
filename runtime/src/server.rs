...
impl<H, Fut> Nea<H>
where
    H: Copy + Fn(TcpStream) -> Fut + Send + Sync + 'static,
        ...
        for (i, stream) in listener.incoming().enumerate() {
            ...
            match executor.try_claim() {
                ...
                Some(index) => executor.execute(index, async move {
                    info!("new connection {i} (index = {}, fd = {fd})", index.index);

                    let queue_index = QueueIndex::from_bucket_index(config.io_resources, index);
                    let tcp_stream = reactor.register(queue_index, stream).unwrap();

                    match (self.handler)(tcp_stream).await {
                        Ok(()) => {}
                        Err(e) => match e.kind() {
